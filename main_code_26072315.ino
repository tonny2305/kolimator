...
// PERBAIKAN: KONTROL MOTOR BERBASIS TARGET POTENSIOMETER
// -----------------------------------------------------------------------------
...
// PERBAIKAN: Motor sekarang bergerak sampai target potensiometer tercapai, bukan hanya dalam waktu tertentu.
...
#define YREAD1    A2
#define YREAD2    A3
#define toleransi 50  // ← Atur nilai ini (semakin kecil = semakin presisi)

// Target nilai potensiometer untuk setiap ukuran film
#define TARGET_18x24_X1  250  // ← Atur nilai ini
#define TARGET_18x24_X2  250  // ← Atur nilai ini
#define TARGET_18x24_Y1  250  // ← Atur nilai ini
#define TARGET_18x24_Y2  250  // ← Atur nilai ini

#define TARGET_24x30_X1  350  // ← Atur nilai ini
#define TARGET_24x30_X2  350  // ← Atur nilai ini
#define TARGET_24x30_Y1  350  // ← Atur nilai ini
#define TARGET_24x30_Y2  350  // ← Atur nilai ini

#define TARGET_30x40_X1  450  // ← Atur nilai ini
#define TARGET_30x40_X2  450  // ← Atur nilai ini
#define TARGET_30x40_Y1  450  // ← Atur nilai ini
#define TARGET_30x40_Y2  450  // ← Atur nilai ini

#define TARGET_35x43_X1  550  // ← Atur nilai ini
#define TARGET_35x43_X2  550  // ← Atur nilai ini
#define TARGET_35x43_Y1  550  // ← Atur nilai ini
#define TARGET_35x43_Y2  550  // ← Atur nilai ini

// Target untuk iluminasi (25x25 cm)
#define TARGET_ILUMINASI_X1  250  // ← Atur nilai ini
#define TARGET_ILUMINASI_X2  250  // ← Atur nilai ini
#define TARGET_ILUMINASI_Y1  250  // ← Atur nilai ini
#define TARGET_ILUMINASI_Y2  250  // ← Atur nilai ini

#define OPEN_X1  900
#define CLOSE_X1  100  // ← Atur nilai ini
#define OPEN_X2  900
#define CLOSE_X2  100  // ← Atur nilai ini
#define OPEN_Y1  900
#define CLOSE_Y1  100  // ← Atur nilai ini
#define OPEN_Y2  900
#define CLOSE_Y2  100  // ← Atur nilai ini
...
void kontrolMotorSampaiTarget(int targetX1, int targetX2, int targetY1, int targetY2) {
  ...
  const unsigned long timeout = 30000; // 30 detik timeout ← Atur nilai ini
  ...
  if (touch.touched()) {
    ...
    if (!menungguPilihanUkuran && !sedangKolimasi && !menungguValidasiIluminasi && !menungguAksiSetelahKolimasi) {
      ...
      for (int i = 0; i < 4; i++) {
        int row = i / 2;
        int col = i % 2;
        int x0 = marginX + col * (boxW + gapX);
        int y0 = marginY + row * (boxH + gapY);
        if (x > x0 && x < x0 + boxW && y > y0 && y < y0 + boxH) {
          ukuran = i;
          selesaiKolimasi();
        }
      }
      // Tombol kembali ke menu utama
      if (x > 40 && x < 280 && y > 260 && y < 310) {
        tampilMenu();
      ...
      if (y > 150 && y < 210) {  // Tercapai button area
        validasiIluminasi(true);
      } 
      else if (y > 230 && y < 290) {  // Tidak button area
        validasiIluminasi(false);
      } else if (y > 310 && y < 370) { // Kembali button area
...
void selesaiKolimasi() {
  ...
  // Mengontrol motor sampai target tercapai berdasarkan ukuran film yang dipilih
